...
function collisions:generateAABB()
    ...
    for _,vert in ipairs(self.verts) do
        dd = aabb.min[1]
        if dd == nil then dd = 0 end
        cc = vert[1]
        if cc == nil then cc = 0 end

        aabb.min[1] = math.min(dd, cc)
        dd = aabb.min[2]
        if dd == nil then dd = 0 end
        cc = vert[2]
        if cc == nil then cc = 0 end

        aabb.min[2] = math.min(dd, cc)
        dd = aabb.min[3]
        if dd == nil then dd = 0 end
        cc = vert[3]
        if cc == nil then cc = 0 end

        aabb.min[3] = math.min(dd, cc)
        dd = aabb.max[1]
        if dd == nil then dd = 0 end
        cc = vert[1]
        if cc == nil then cc = 0 end

        aabb.max[1] = math.max(dd, cc)
        dd = aabb.max[2]
        if dd == nil then dd = 0 end
        cc = vert[2]
        if cc == nil then cc = 0 end

        aabb.max[2] = math.max(dd, cc)
        dd = aabb.max[3]
        if dd == nil then dd = 0 end
        cc = vert[3]
        if cc == nil then cc = 0 end

        aabb.max[3] = math.max(dd, cc)
